...
// } // создали коллекцию из 9 изображений с енотами
...
//     debugger
//     el.querySelector('img').setAttribute('src', `images/${imagePosition[index]}.png`)

//     // el.querySelector('img').setAttribute('src', `images/${getRandomIntInclusive(1, 6)}.png`);
//     el.isOpen = false;
// })
...
<div class="card">
    ...
    <div class="back active"><img src="images/back.png" alt="racoon image">
</div>`;
...
function flipCard(card) {
    if (canflip) { // latency 0.5s

        card.classList.toggle('flipped');
        card.isFlipped = !card.isFlipped;

        setTimeout(() => {
            card.querySelector('.front').classList.toggle('active');
            card.querySelector('.back').classList.toggle('active');
        }, 100)
...
function getAnimationDuration() {
    return String(Math.random() + 0.9).slice(0, 7)
}
